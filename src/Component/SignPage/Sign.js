...
function Sign() {
    ...
    const getDataFilter=()=>{
        axios.get("https://3mzyme0mm9.execute-api.ap-south-1.amazonaws.com/test/ApiAuth")
    ...
    const handleSubmitRegister=(e)=>{
        e.preventDefault()
        axios.post(`https://3mzyme0mm9.execute-api.ap-south-1.amazonaws.com/test/ApiAuth`,regiterDat)
        .then(res=>{
            seNext(!next)
        })
    }
    const handleSubmit=(e)=>{
        ...
        axios.get(`https://3mzyme0mm9.execute-api.ap-south-1.amazonaws.com/test/ApiAuth?email=${email}&password=${password}`)
        .then(res=>{
            const data=res.data
            if(data.length==0){
                setAuth(!auth)
